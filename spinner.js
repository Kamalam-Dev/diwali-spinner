...
const GOOGLE_SHEET_URL="YOUR_WEB_APP_URL_HERE"; // <-- replace this
...
for(let i=0;i<numSegments;i++){
    ...
    label.style.transform=`rotate(${360/numSegments/2}deg)`;
...
function launchConfetti(){
    ...
    for(let i=0;i<120;i++){
        confettiParticles.push({x:Math.random()*canvas.width, y:Math.random()*canvas.height-canvas.height, r:Math.random()*5+2, d:Math.random()*10+5, color:confColors[Math.floor(Math.random()*confColors.length)], tilt:Math.random()*10-10, tiltAngleIncrement:Math.random()*0.05+0.02, type:Math.random()<0.3?'spark':'circle'});
    }
